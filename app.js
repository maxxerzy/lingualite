...
const STORAGE_KEY='lingualite_state_v22';
...
/* Decks rendering (fragment + single click handler) */
...
/* Controls: single event bindings */
document.addEventListener('DOMContentLoaded',()=>{
  document.getElementById('startBtn').addEventListener('click', startLearning, {passive:true});
  document.getElementById('reloadBtn').addEventListener('click', reloadExternalDecks, {passive:true});
  document.getElementById('exportBtn').addEventListener('click', exportDecks, {passive:true});
  document.getElementById('importTextBtn').addEventListener('click', importFromText, {passive:true});
  document.getElementById('importFile').addEventListener('change', handleFileImport, {passive:true});
});
...
function showFlashcard(area){
  const card=document.createElement('div'); card.className='card'; card.textContent=currentCard.front; area.appendChild(card);
  const bar=document.createElement('div'); bar.className='action-bar';
...
function showMultiple(area){
  ...
  while(opts.length<4 && pool.length>opts.length){ const c=pool[Math.floor(Math.random()*pool.length)].back; if(!uniq.has(c)){opts.push(c);uniq.add(c);} }
  ...
  opts.forEach(o=> grid.appendChild(mkBtn(o,'primary go',()=>{ const ok=(o===currentCard.back); pushResultAndAwaitNext(area, ok); })));
...
/* --- Sentence Puzzle with BUTTON POOL (re-use nodes) --- */
const buttonPool=[];
function acquireButton(){ return buttonPool.pop() || document.createElement('button'); }
function releaseButton(btn){ btn.disabled=false; btn.removeAttribute('data-w'); btn.textContent=''; btn.onclick=null; buttonPool.push(btn); }
...
function showSentence(area){
  ...
  const sourceWords=source.split(/\s+/).map(w=>w.replace(PUNC,''));
  ...
  const prompt=document.createElement('div'); prompt.className='card'; prompt.innerHTML='<b>Deutsch:</b> '+currentCard.front;
  ...
  for(let i=0;i<bank.length;i++){
    const b=acquireButton();
    b.setAttribute('data-w', bank[i]);
    b.textContent=bank[i];
    b.disabled=false;
    frag.appendChild(b);
  }
  wb.appendChild(frag);
  ...
  const btnPruefen=mkBtn('Prüfen','primary go',()=>{
    ...
    const ok=(levenshtein(normalize(val), normalize(target)) <= Math.max(1, Math.floor(normalize(target).length*0.05)));
    pushResultAndAwaitNext(area, ok);
    btnPruefen.remove();
  });
  ...
  // Release buttons back to pool when moving on
  const observer=new MutationObserver(()=>{
    if(!document.body.contains(area)){ // safety
      wb.querySelectorAll('button').forEach(releaseButton);
      observer.disconnect();
    }
  });
