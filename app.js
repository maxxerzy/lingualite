...
const STORAGE_KEY='lingualite_state_v30';
const ELO_KEY='lingualite_elo_v30';
...
// Embedded fallback with 3x1000 cards (populated from decks.json file shipped)
const FALLBACK_DECKS = null; // will be injected at runtime if fetch fails (see bootstrap)
...
let ELO=loadJSON(ELO_KEY,{decks:{}, cards:{}, user:DEFAULT_ELO, rr:{}});
...
// Guaranteed rotation state
let rrIndex=0;        // pointer into deck for fairness
let stepsSinceBoost=0;
const FAIRNESS_INTERVAL=15; // every N answers, inject long-not-seen card
...
async function loadExternalDecks(){
  ...
  // Fallback: try window.FALLBACK_DECKS if defined
  if(window.FALLBACK_DECKS && Array.isArray(window.FALLBACK_DECKS.decks)){
    const box=document.getElementById('deckError');
    if(box){ box.style.display='block'; box.textContent='Hinweis: Offline‑Decks aktiv (decks.json nicht gefunden).'; }
    return window.FALLBACK_DECKS.decks;
  }
  const box=document.getElementById('deckError');
  if(box){ box.style.display='block'; box.textContent='Fehler: Konnte keine Decks laden.'; }
  return [];
}

async function bootstrap(){
  if(!decks){
    // try to load decks.json
    decks=await loadExternalDecks();
    STATE.decks=decks; saveJSON(STORAGE_KEY,STATE);
  }
...
function startLearning(){
  ...
  if(ELO.rr[currentDeck.name]==null) ELO.rr[currentDeck.name]=0; // persist round-robin pointer
  rrIndex=ELO.rr[currentDeck.name]|0;
  score=0; wrongStreak=0; stepsSinceBoost=0; updateMeters();

  // build sessionQueue with ids
  sessionQueue=currentDeck.cards.map(c=>({ ...c, _id: hashCard(currentDeck.name,c), _lastSeen: 0 }));
  // sort by card elo desc (harder first)
  sessionQueue.sort((a,b)=> (getCardElo(b._id)-getCardElo(a._id)));

  nextCard();
}

function fairnessInject(){
  // inject the next round-robin card that is not already within next ~8 positions
  const N=sessionQueue.length;
  if(N===0) return;
  rrIndex = (rrIndex+1) % N;
  // pick candidate far from the front to avoid duplicates; move it near front
  const idx = rrIndex;
  const candidate = sessionQueue.splice(idx,1)[0];
  sessionQueue.splice(Math.min(3, sessionQueue.length), 0, candidate);
  ELO.rr[currentDeck.name]=rrIndex; saveJSON(ELO_KEY,ELO);
}
...
function nextCard(){
  ...
  // fairness boost every FAIRNESS_INTERVAL steps
  if(stepsSinceBoost>=FAIRNESS_INTERVAL){
    fairnessInject();
    stepsSinceBoost=0;
  }
...
function onAnswer(ok){
  ...
  const {newA: deckElo2} = eloUpdate(deckElo, userElo, ok?0:1);
  setUserElo(userElo2); setCardElo(currentCard._id, clamp(cardElo2)); ELO.decks[currentDeck.name]=clamp(deckElo2);
  updateMeters();
  // scheduling
  if(ok){ sessionQueue.splice(Math.min(6+Math.floor(Math.random()*2),sessionQueue.length),0,currentCard); }
...
function showSentence(area){
  ...
  const MAX_BANK=24; const baseTarget=tWords.map(w=>w.toLowerCase());
  const sourceWords=front.split(/\s+/).map(w=>w.replace(PUNC,'')); const srcPool=sourceWords.filter(w=>w && !baseTarget.includes(w.toLowerCase()));
