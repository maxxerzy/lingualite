...
const STORAGE_KEY='lingualite_state_v17';
...
function pushResultAndAwaitNext(ok,msgOk='✅ Richtig!',msgFail='❌ Falsch!'){ const area=document.getElementById('learningArea'); const fb=document.createElement('div'); fb.className='feedback'; fb.innerHTML=ok?msgOk:msgFail+' Richtige Lösung: <b>'+currentCard.back+'</b>'; area.appendChild(fb); const bar=document.createElement('div'); bar.className='action-bar'; bar.appendChild(mkBtn('Weiter','primary next',()=>{ if(ok){const offset=6+Math.floor(Math.random()*2); sessionQueue.splice(Math.min(offset,sessionQueue.length),0,currentCard);} else { sessionQueue.splice(1,0,currentCard);} nextCard(); })); area.appendChild(bar); }

/* Flashcards */
function showFlashcard(){ const area=document.getElementById('learningArea'); area.innerHTML='<div class=card>'+currentCard.front+'</div>'; const bar=document.createElement('div'); bar.className='action-bar'; bar.appendChild(mkBtn('Antwort zeigen','primary go reveal',()=>{ const fb=document.createElement('div'); fb.className='feedback'; fb.innerHTML='Richtig: <b>'+currentCard.back+'</b>'; area.appendChild(fb); const inner=document.createElement('div'); inner.className='action-bar'; inner.appendChild(mkBtn('Richtig','primary go',()=>pushResultAndAwaitNext(true))); inner.appendChild(mkBtn('Falsch','primary go',()=>pushResultAndAwaitNext(false))); area.appendChild(inner); })); bar.appendChild(mkBtn('Skip','skip',()=>{ sessionQueue.push(currentCard); nextCard(); })); area.appendChild(bar); }

/* Multiple Choice: ≥4 Optionen */
function showMultiple(){ const pool=currentDeck.cards; const opts=[currentCard.back]; while(opts.length<4 && pool.length>opts.length){ const c=pool[Math.floor(Math.random()*pool.length)].back; if(!opts.includes(c)) opts.push(c); } while(opts.length<4){ const c=DISTRACTOR_POOL[Math.floor(Math.random()*DISTRACTOR_POOL.length)]; if(!opts.includes(c)) opts.push(c); } opts.sort(()=>Math.random()-.5); const area=document.getElementById('learningArea'); area.innerHTML='<div class=card>'+currentCard.front+'</div>'; const grid=document.createElement('div'); grid.className='action-bar'; opts.forEach(o=> grid.appendChild(mkBtn(o,'primary go',()=>{ const ok=(o===currentCard.back); pushResultAndAwaitNext(ok); }))); area.appendChild(grid); const bar=document.createElement('div'); bar.className='action-bar'; bar.appendChild(mkBtn('Skip','skip',()=>{ sessionQueue.push(currentCard); nextCard(); })); area.appendChild(bar); }
...
/* Satzpuzzle – Wort-für-Wort; mind. 6 Distraktoren; Ein-Wort-Ziel => automatisch MC; Prüfen -> Weiter; leer => Skip-Hinweis */
function showSentence(){
  const target=String(currentCard.back||'').trim();
  const words=target.split(/\\s+/);
  if(words.length<=1){ showMultiple(); return; }
  const needed=Math.min(12,Math.max(6,Math.ceil(words.length*0.6)));
  const distractors=new Set();
  const base=words.map(w=>w.toLowerCase());
  while(distractors.size<needed){
    const w=DISTRACTOR_POOL[Math.floor(Math.random()*DISTRACTOR_POOL.length)];
    if(!base.includes(w.toLowerCase())) distractors.add(w);
  }
